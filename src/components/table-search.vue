    ...
    <div class="search-container">
        <el-form :model="query" :inline="true">
            <el-form-item v-for="item in options" :key="item.prop" :label="item.label" :prop="item.prop">
                <el-input v-if="item.type == 'input'" v-model="query[item.prop]" :placeholder="item.placeholder" :disabled="item.disabled"  clearable/>
                <el-select v-else-if="item.type == 'select'" v-model="query[item.prop]"  :placeholder="item.placeholder" :disabled="item.disabled" clearable @change="handleSelectChange" style="width: 100px">
