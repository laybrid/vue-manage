            ...
            <el-col :span="span" v-for="item in options" :key="item.prop">
                <el-form-item :label="item.label" :prop="item.prop">
                    ...
                    <el-upload v-else-if="item.type === 'upload'" class="avatar-uploader" action="#"
                        ...
                        <img v-if="formData[item.prop]" :src="formData[item.prop] as string" class="avatar" />
...
// fix 目前先不用rules
const rules: FormRules = props.options.map(item => {
    if (item.required) {
...
const saveEdit = (formEl: FormInstance | undefined) => {
    ...
    formEl.validate(async valid => {
        // fix upload
        // if (!valid) return;
        props.edit ? await updateTableList(props.formData) : await addTableList(props.formData);
        props.update();
    });
};
</script>
